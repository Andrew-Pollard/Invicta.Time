...
- **Armed for the earliest deadline:** a thread scheduling a sooner timer re-arms the waitable timer itself, so no
...
- **First timer in a process:** the first 1 ms callback arrives after about 6 ms, against 1.1 ms once warm, while the
  code is jitted, the waitable timer is created and the scheduler thread starts. Await a short `Task.Delay` on the
